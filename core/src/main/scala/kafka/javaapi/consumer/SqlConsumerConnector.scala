...
import kafka.common.{MessageStreamsExistException, OffsetAndMetadata, TopicAndPartition}
...
private[kafka] class SqlConsumerConnector(val config: ConsumerConfig,
                                          val topicsAndQueries: java.util.Map[String, String],
  ...
  def this(config: ConsumerConfig, topicsAndQueries: java.util.Map[String, String]) =
    this(config = config, topicsAndQueries = topicsAndQueries,
      enableFetcher = true)
  ...
  def createMessageStreamsByFilter(topicFilter: TopicFilter, numStreams: Int) = null

  def createMessageStreamsByFilter(topicFilter: TopicFilter) = null
