...
/**
  * Created by ericfalk on 11/01/16.
  */
private[kafka] object SqlConsumerConnector {
...
//    wildcardStreamsHandler.streams
  ...
  private def registerConsumerSqlQueries(dirs: ZKSqlQueryDirs, consumerIdString: String, topicsAndQueries: Map[String,String]) = {
    info("begin registering consumer queries for consumer " + consumerIdString + "in ZK")
    val consumerSqlQueries = Json.encode(topicsAndQueries)
    val zkWatchedEphemeral = new ZKCheckedEphemeral(dirs.consumerDir, consumerSqlQueries, zkUtils.zkConnection.getZookeeper,false)
    zkWatchedEphemeral.create()

    info("end registering consumer queries for consumer " + consumerIdString + " in ZK")
  }
  ...
  class ZKSessionExpireListener(val dirs: ZKGroupDirs,
                                val sqlDirs: ZKSqlQueryDirs,
                                val consumerIdString: String,
                                val topicCount: TopicCount,
                                val loadBalancerListener: ZKRebalancerListener,
                                val topicsAndQueries: Map[String,String])
  ...
  private def reinitializeConsumer[K,V]( topicCount: TopicCount,
    ...
    val consumerThreadIdsPerTopic: Map[String, collection.Set[ConsumerThreadId]] = topicCount.getConsumerThreadIdsPerTopic
